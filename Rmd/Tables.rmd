...
anim_394 <- event_394 %>%
  ...
  mutate(value = case_when(player_id == -1 ~ "ball",
                           player_id != -1 ~ "player"),
         team_id = factor(team_id)) # Need to make sure this encodes by defense or offense later
...
p2 <- fullcourt() +
  ...
  geom_point(data = chull_394, aes(x = centroid_x, y = centroid_y, group = team_id, color = team_id),
             size = 3, shape = 13) +
  geom_text(data = chull_394, aes(x = centroid_x, y = centroid_y, group = team_id, color = team_id),
             size = 3, label = "Centroid:", hjust = 1.2, show.legend = F) +
  labs(title = "{closest_state}") # Needs fixing this literally doesn't ever load???
anim_test <- p2 + transition_states(states = group_id)
animated_test <- animate(anim_test, 
                    width = 800,
                    height = 800,
                    nframes = length(unique(anim_394$group_id)),
                    fps = 10,
                    res = 150,
                    end_pause = 0)
anim_save(here("Gifs/394_0021500491.gif"), animated_test)
```
